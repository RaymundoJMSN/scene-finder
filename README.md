...
![versão](https://img.shields.io/badge/vers%C3%A3o-1.0.0-4ade80) ![licença](https://img.shields.io/badge/licen%C3%A7a-MIT-blue)
...
consulta em português e o arquivo em inglês. Clicar num resultado copia o caminho já no formato do
Foundry; o botão de pasta abre o Explorer no arquivo.
...
O app avisa quando há versão nova, baixa e abre o instalador — seu índice e configurações são
preservados.

## Como a busca local funciona

Um modelo CLIP sozinho não resolve battle maps: vistos de cima, quase todos parecem iguais para ele,
os scores empatam por volta de 0,30 e o primeiro resultado vira sorte. O ranking aqui soma três
sinais:

1. **Semântica da imagem** — CLIP ViT-B/32 sobre o conteúdo visual.
2. **Semântica do nome do arquivo** — embedado no mesmo espaço vetorial por um modelo de texto
...
Os modelos rodam em ONNX quantizado (int8), no processador. Sem GPU, sem nuvem: nenhuma imagem sua
sai do computador — só o texto da busca vai para as fontes online.
...
venv\Scripts\python tools\verify_onnx.py    # ONNX não degradou o ranking
```
...
| `encoder.py` | Inferência CLIP em ONNX |
